...
foreach ($name in 'controller','receiver','sender') { if (-not (Test-Path -LiteralPath (Join-Path $bin "$name.exe"))) { throw "Missing bin\$name.exe. Run scripts\build-all.ps1 first." } }
